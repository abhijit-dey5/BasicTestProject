...
Puppet::Type.type(:gauth_credential).provide(:defaultuserapplication) do
  has_feature :defaultuserapplication
  ...
  def self.instances
    @resource_collector[:gauth_credential].select do |resource|
      resource.provider == 'defaultuserapplication'
    end.map(&:provider)
  end
  ...
  def authorization
    ...
    Google::Authorization.new.from_application_default_credentials!
  end
end
